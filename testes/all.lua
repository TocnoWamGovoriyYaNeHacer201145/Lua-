...
do
  ...
  show(string.format("random seeds: %d, %d", random_x, random_y))
end
...
local T,show,format,write,assert,type,unpack,floor =
      T,show,string.format,io.write,assert,type,table.unpack,math.floor
...
if not T then
  ...
  showmem = function ()
    ...
    show(format("    ---- total memory: %s, max memory: %s ----\n",
          F(m), F(max)))
  end
  ...
  showmem = function ()
    ...
    show(format("\t(strings:  %d, tables: %d, functions: %d, "..
                 "\n\tudata: %d, threads: %d)",
...
--
local function report (n) show("\n***** FILE '"..n.."'*****") end
local olddofile = dofile
local dofile = function (n, strip)
  showmem()
  local c = os.clock()
  show(string.format("time: %g (+%g)", c - initclock, c - lastclock))
  lastclock = c
  report(n)
...
show("(there should be two warnings now)")
warn("@on")
...
show(string.format("%d-bit integers, %d-bit floats",
...
local _G, showmem, show, format, clock, time, difftime,
      assert, open, warn =
      _G, showmem, show, string.format, os.clock, os.time, os.difftime,
      assert, io.open, warn
